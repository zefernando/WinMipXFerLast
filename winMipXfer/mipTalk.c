...
//		Run->Block_Count,Buffer = atoi(auxB);
			...
			Run->Block_Count =   auxB[1] << 16 |  auxB[2] << 8 | auxB[3];
	            sprintf(Log_Msg,"*MIP REPORTED file BLOCK COUNT as %ld Blocks",
                        Run->Block_Count);
                Log_This(Log_Msg, mpLOG_NORMAL);
        }
	 ...
	 unsigned char auxB[5];
        ...
        while (1) {
                ...
                case stRECEIVING:
					...
					if(Run->bTrim == 1) {
						Len = stripBlanks(Buffer, Len);
                                ...
                                if(Run->bASCII_Output)
                                {
						if(Run->bKeepBlanks == 1) {
							noBlanks =  stripBlanks(Buffer, Len);
                                        	/* Converte saida para ASCII */
                                        	conv_ascii(Buffer+1,Buffer+1,noBlanks-1);
						}
						else {
			    ...
			    if(Len > 0)
						...
						sprintf(Log_Msg,"*Bytes written  now %d  - Total %ld", rc, Run->Bytes_Received);
                                        Log_This(Log_Msg,mpLOG_NORMAL);
                                        /* incrementa o número de blocos recebidos */
                                        Run->Block_Number++;
						sprintf(Log_Msg,"*Block number is %ld", Run->Block_Number);
                                        Log_This(Log_Msg,mpLOG_NORMAL);
                                }
                        }
                        else
                ...
                case stENDING:
				      ...
				      mipBlocks =   auxB[1] << 16 |  auxB[2] << 8 | auxB[3];
                                sprintf(Log_Msg,"*MIP reports %ld blocks sent",mipBlocks);
                                Log_This(Log_Msg,mpLOG_NORMAL);
                                /* Numero de blocos computado pelo mipXfer */
                                sprintf(Log_Msg,"*HOST reports %ld blocks read",
                                        Run->Block_Number + 1);
	 				Log_This(Log_Msg,mpLOG_NORMAL);
					sprintf(Log_Msg,"Purge is set to %d", Run->bPurge);
       	                 	Log_This(Log_Msg, mpLOG_NORMAL);

					if(Run->bPurge == 1)
					{
     	                      		sprintf(Log_Msg,"Purge is on");
       	                 		Log_This(Log_Msg, mpLOG_NORMAL);
 						
        		                    if(mipBlocks == ( 1 + Run->Block_Number))
                              		{
...
int stripBlanks(unsigned char * bp, int len)
{
	...
	newLen = len;
	count = 0;
	sprintf(Log_Msg, "Counting blanks");
	Log_This(Log_Msg, mpLOG_NORMAL);
	do {
		if(*ptr == 0x40) { 
		 --newLen;
